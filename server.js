...
async function deleteUserData(userId) {
  ...
  for (const a of avatars) {
    if (a.avatar_path && a.avatar_path.startsWith('http')) {
      try { await cloudinary.uploader.destroy(a.avatar_path); } catch (e) {}
    }
  }

  const files = await dbAll("SELECT filename FROM files WHERE user_id=?", [userId]);
  for (const f of files) {
    if (f.filename && f.filename.startsWith('http')) {
      try { await cloudinary.uploader.destroy(f.filename); } catch (e) {}
    }
  }
  ...
  for (const f of messageFiles) {
    if (f.file_path && f.file_path.startsWith('http')) {
      try { await cloudinary.uploader.destroy(f.file_path); } catch (e) {}
    }
  }
...
app.delete('/api/avatars/:id', auth, async function(req, res) {
  ...
  if (avatar.avatar_path && avatar.avatar_path.startsWith('http')) {
    try { await cloudinary.uploader.destroy(avatar.avatar_path); } catch (e) {}
  }
...
app.post('/api/messages/:fid', auth, upload.single('file'), async function(req, res) {
  try {
    ...
    if (req.file) {
      ...
      durationSeconds = durationSeconds || req.file.duration || null;
      mediaKind = fileType.startsWith('video/') ? 'video' : 
...
app.post('/api/messages/:id/destruct', auth, async function(req, res) {
  ...
  if (msg.is_self_destruct && msg.receiver_id === req.userId) {
    if (msg.file_path && msg.file_path.startsWith('http')) {
      try { await cloudinary.uploader.destroy(msg.file_path); } catch (e) {}
    }
